...
function TeacherSidebar({ activeTab, onTabChange, isOpen, onToggle, onLogout }) {
  const menuItems = [
    { id: 'overview', icon: FiGrid, label: 'Overview' },
    { id: 'students', icon: FiUsers, label: 'My Students' },
    { id: 'projects', icon: FiFolder, label: 'Projects' },
    { id: 'requests', icon: FiClock, label: 'Requests' },
    { id: 'students-reports',  icon: FiBarChart2, label: 'Reports' },
      ...
      `}>
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-gray-200 sticky top-0 bg-white z-10">
          <div className={`flex items-center gap-2 ${!isOpen && 'justify-center w-full'}`}>
            ...
            {isOpen && (
              <div>
                <span className="font-bold text-gray-800 text-lg">Teacher Panel</span>
